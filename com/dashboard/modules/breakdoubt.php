...
class breakdoubt extends App{
	...
	function beforeFilter(){
		...
		$this->assign('gamestitle', " Break Doubt" );
	}
	...
	function main(){
		$this->assign('uid',strip_tags($this->_g('uid')));
			...
			$totalquery =  $this->userHelper->getgamestrackall(2);
			// pr($totalquery);
			...
			$games = $this->userHelper->getgamestrack(2);
	...
	function entourage(){
			...
			$games = $this->userHelper->getgamestrackentourage(2);
			$this->assign('games',$games);
		if(strip_tags($this->_g('page'))=='home') $this->log('surf','home');
