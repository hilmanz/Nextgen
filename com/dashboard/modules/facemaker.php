...
class facemaker extends App{
	...
	function main(){
		$this->assign('uid',strip_tags($this->_g('uid')));
			...
			$totalquery =  $this->userHelper->getgamestrackall(3);
			// pr($totalquery);
			...
			$games = $this->userHelper->getgamestrack(3);
		...
		return $this->View->toString(TEMPLATE_DOMAIN_DASHBOARD .'apps/facemaker-pages.html');
	...
	function entourage(){
			...
			$games = $this->userHelper->getgamestrackentourage(3);
			$this->assign('games',$games);
		if(strip_tags($this->_g('page'))=='home') $this->log('surf','home');
